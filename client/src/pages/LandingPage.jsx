...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#fffdf8] text-slate-950">
      <header className="sticky top-0 z-40 border-b border-slate-900/10 bg-[#fffdf8]/95 backdrop-blur">
        <nav className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-4 py-4 sm:px-6 lg:px-8" aria-label="Landing navigation">
          ...
          <div className="hidden items-center gap-4 sm:flex">
            <Link className="text-sm font-semibold text-slate-700 hover:text-slate-950" to="/login">Log in</Link>
            <Link className="rounded-md bg-[#1e4d3a] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#173d2e]" to={primaryPath}>{isAuthenticated ? "Open dashboard" : "Start tracking"}</Link>
          </div>
        ...
        {menuOpen ? <div className="border-t border-slate-200 bg-[#fffdf8] px-4 py-4 sm:hidden"><div className="mx-auto flex max-w-7xl flex-col gap-1">{navItems.map((item) => <a className="rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-white" href={item.href} key={item.label} onClick={closeMenu}>{item.label}</a>)}<Link className="mt-2 rounded-md border border-slate-300 px-3 py-2 text-center text-sm font-semibold text-slate-700" onClick={closeMenu} to="/login">Log in</Link><Link className="rounded-md bg-[#1e4d3a] px-3 py-2 text-center text-sm font-semibold text-white" onClick={closeMenu} to={primaryPath}>{isAuthenticated ? "Open dashboard" : "Start tracking"}</Link></div></div> : null}
      </header>
      ...
      <section className="border-b border-slate-200 bg-[#fffdf8]">
        <div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 md:py-20 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-8 lg:py-24">
          <div className="max-w-2xl">
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-slate-600">Track income, expenses, habits, savings goals, and net worth in one calm financial dashboard.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link className="inline-flex items-center justify-center gap-2 rounded-md bg-[#1e4d3a] px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#173d2e]" to={primaryPath}>{isAuthenticated ? "Open your dashboard" : "Start tracking"}<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><a className="inline-flex items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:border-slate-400" href="#features">Explore features<BarChart3 className="h-4 w-4" aria-hidden="true" /></a></div>
      ...
      <section className="bg-[#e6f3eb] py-20"><div className="mx-auto max-w-3xl px-4 text-center sm:px-6"><p className="text-sm font-bold uppercase tracking-wide text-[#1e4d3a]">Start with today</p><h2 className="mt-3 text-3xl font-bold leading-tight text-slate-950 sm:text-4xl">Your money habits become your money future.</h2><p className="mt-4 text-base leading-7 text-slate-600">Start tracking the choices that make your goals possible.</p><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><Link className="inline-flex items-center justify-center gap-2 rounded-md bg-[#1e4d3a] px-5 py-3 text-sm font-bold text-white hover:bg-[#173d2e]" to={primaryPath}>{isAuthenticated ? "Open dashboard" : "Start tracking"}<ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><Link className="inline-flex items-center justify-center rounded-md border border-[#1e4d3a] px-5 py-3 text-sm font-bold text-[#1e4d3a] hover:bg-white/60" to="/login">Log in</Link></div></div></section>

      <footer className="bg-slate-950 py-12 text-slate-300"><div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 md:grid-cols-[1fr_auto] lg:px-8"><div><Link className="flex items-center gap-2 text-lg font-bold text-white" to="/"><span className="grid h-8 w-8 place-items-center rounded-md bg-[#f4c95d] text-sm font-extrabold text-slate-950">W</span>WealthTrack</Link><p className="mt-4 max-w-md text-sm leading-6 text-slate-400">A financial habit and wealth growth tracker for clearer spending, stronger routines, and progress you can see.</p></div><div className="flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold"><Link className="hover:text-white" to={isAuthenticated ? "/dashboard" : "/login"}>Dashboard</Link><a className="hover:text-white" href="#features">Features</a><a className="hover:text-white" href="#goals">Goals</a><Link className="hover:text-white" to="/login">Log in</Link></div></div><div className="mx-auto mt-10 max-w-7xl border-t border-white/10 px-4 pt-6 text-xs text-slate-500 sm:px-6 lg:px-8">Built as a financial habit builder and wealth growth tracker.</div></footer>
    </main>
  );
}
